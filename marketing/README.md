...
src/data/instructions.json   generated by scripts/instructions.mjs from the VM source
scripts/instructions.mjs     the generator, with a --check mode the build runs
...
`src/data/instructions.json` is written by `npm run gen` from `crates/foxvm/src/bytecode.rs`:
the `Instr` enum's `// ---- name` markers become the groups, each variant's doc comment its
description, and a struct variant's fields its operands. `npm run build` runs `gen:check`
first and refuses to build when the file is stale, so after changing the enum run `npm run gen`
and commit the result. `src/pages/docs/instructions.astro` renders it.
